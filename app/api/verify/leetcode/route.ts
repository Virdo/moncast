import { fetchLeetCodeProfile } from "@/lib/platform-verification";
...
export async function POST(request: Request) {
  ...
  if (!validProviderHandle(body.username)) {
    return Response.json({ error: "INVALID_USERNAME" }, { status: 400 });
  }

  try {
    const profile = await fetchLeetCodeProfile(body.username, body.utcOffsetMinutes);
